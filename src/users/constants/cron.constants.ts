...
export const NOTIFICATION_MESSAGES = {
  ...
  BIRTHDAY_REMINDER: {
    REFERRER: (fullName: string, days: number) =>
      `Sinh nhật của ${fullName} sẽ diễn ra trong ${days} ngày nữa. Đừng quên gửi lời chúc mừng!`,
    DIRECT_REFERRAL: (fullName: string, days: number) =>
      `Sinh nhật của người giới thiệu ${fullName} sẽ diễn ra trong ${days} ngày nữa. Hãy gửi lời chúc mừng để thể hiện sự biết ơn!`,
    INDIRECT_REFERRAL: (fullName: string, days: number) =>
      `Sinh nhật của ${fullName} trong mạng lưới giới thiệu sẽ diễn ra trong ${days} ngày nữa. Hãy gửi lời chúc mừng!`,
  },
} as const;
